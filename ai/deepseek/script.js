document.addEventListener('DOMContentLoaded', () => {
    ...
    document.body.addEventListener('click', (e) => {
        ...
        // If link is a modal trigger (Join Now), open modal and skip SPA navigation
        if (link.id === 'join-now-btn') {
            e.preventDefault();
            openModal();
            return;
        }

        // If link has .no-spa, allow default navigation
        if (link.classList.contains('no-spa')) return;

        // If link is internal and not a hash, handle SPA navigation
        if (
            link.origin === window.location.origin &&
            !link.hash &&
            !link.id?.includes('show-') // skip modal view switchers
        ) {
            e.preventDefault();
    ...
    // Make sure openModal is globally available
    ...
    const toggleLoading = (button, isLoading) => {
        ...
        buttonText.textContent = isLoading ? 'Processing...' : buttonText.dataset.originalText;
    };

    const handleFormSubmit = async (form, isLogin = false) => {
    ...
    if (authModal && joinNowBtn) {
        ...
        const openModal = (view = 'signup') => {
            ...
            // Reset form fields
            document.querySelectorAll('#signup-form input, #login-form input').forEach(input => {
                input.value = '';
            });
        };
        ...
        document.getElementById('signup-form').addEventListener('submit', async (e) => {
            e.preventDefault();
            await handleFormSubmit(e.target, false);
        });

        document.getElementById('login-form').addEventListener('submit', async (e) => {
            e.preventDefault();
            await handleFormSubmit(e.target, true);
        });
    }
